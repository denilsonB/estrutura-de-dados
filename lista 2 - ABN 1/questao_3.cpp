...
void retirar(auto produto_pesquisado, vector<produto> &estoque){//funcao de retirar produto do estoque
    int retirada,qtd_total=0;
    
    for(int i=0;i<estoque.size();i++){//contar a quantidade total de produtos 
        if(produto_pesquisado[i].codigo!=produto_pesquisado[0].codigo) break;

        qtd_total += produto_pesquisado[i].quantidade;
    }
    ...
    try{
        ...
        if(retirada>qtd_total) throw -1;//se o usuario pedir mais do que tem no estoque da erro
    }catch(...){
...
>>>>>>> 449b1487fe80fa7ed7049d081868ee5fff7ef4b6:lista 2 - ABN 1/questao_3.cpp
}